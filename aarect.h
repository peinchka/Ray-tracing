...
bool xy_rect::hit(const ray& r, float t0, float t1, hit_record& rec) const {
	float t = (k - r.ray_origin().z()) / r.direction().z();
	if (t < t0 || t > t1)
		return false;
	float x = r.ray_origin().x() + t * r.direction().x();
	float y = r.ray_origin().y() + t * r.direction().y();
...
bool xz_rect::hit(const ray& r, float t0, float t1, hit_record& rec) const {
	float t = (k - r.ray_origin().y()) / r.direction().y();
	if (t < t0 || t > t1)
		return false;
	float x = r.ray_origin().x() + t * r.direction().x();
	float z = r.ray_origin().z() + t * r.direction().z();
...
bool yz_rect::hit(const ray& r, float t0, float t1, hit_record& rec) const {
	float t = (k - r.ray_origin().x()) / r.direction().x();
	if (t < t0 || t > t1)
		return false;
	float y = r.ray_origin().y() + t * r.direction().y();
	float z = r.ray_origin().z() + t * r.direction().z();
