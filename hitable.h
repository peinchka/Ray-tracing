...
bool translate::hit(const ray& r, float t_min, float t_max, hit_record& rec) const {
	ray moved_r(r.ray_origin() - offset, r.direction(), r.time());
...
bool rotate_x::hit(const ray& r, float t_min, float t_max, hit_record& rec) const {
	vec3 origin = r.ray_origin();
	vec3 direction = r.direction();
	origin[1] = cos_theta * r.ray_origin()[1] + sin_theta * r.ray_origin()[2];
	origin[2] = - sin_theta * r.ray_origin()[1] + cos_theta * r.ray_origin()[2];
...
bool rotate_y::hit(const ray& r, float t_min, float t_max, hit_record& rec) const {
	vec3 origin = r.ray_origin();
	vec3 direction = r.direction();
	origin[0] = cos_theta * r.ray_origin()[0] - sin_theta * r.ray_origin()[2];
	origin[2] = sin_theta * r.ray_origin()[0] + cos_theta * r.ray_origin()[2];
...
bool rotate_z::hit(const ray& r, float t_min, float t_max, hit_record& rec) const {
	vec3 origin = r.ray_origin();
	vec3 direction = r.direction();
	origin[0] = cos_theta * r.ray_origin()[0] + sin_theta * r.ray_origin()[1];
	origin[1] = - sin_theta * r.ray_origin()[0] + cos_theta * r.ray_origin()[1];
...
class quat_rotate_x : public hitable {
public:
	quat_rotate_x(hitable* p, float angle);
	virtual bool hit(const ray& r, float t_min, float t_max, hit_record& rec) const;
	virtual bool bounding_box(float t0, float t1, aabb& box) const {
		box = bbox;
		return hasbox;
...
quat_rotate_x::quat_rotate_x(hitable* p, float angle) : ptr(p) {
	float M_PI = 3.141592654f;
	float radians = (M_PI / 360) * angle;
	float s = sin(radians);
	float c = cos(radians);
	cos_theta = c;
	sin_theta = s;
	float sin_squared_theta = s * s;
	float cos_squared_theta = c * c;
	float csminusss = cos_squared_theta - sin_squared_theta;
	float twosincos = 2 * s * c;
	hasbox = ptr->bounding_box(0, 1, bbox);
	float x = bbox.min().x();
	float y = bbox.min().y()*csminusss - twosincos * bbox.min().z();
	float z = bbox.min().z()*csminusss - twosincos * bbox.min().y();
	vec3 min(x, y, z);
	float xx = bbox.max().x();
	float yy = bbox.max().y()*csminusss - twosincos * bbox.max().z();
	float zz = bbox.max().z()*csminusss - twosincos * bbox.max().y();
	vec3 max(xx, yy, zz);
	bbox = aabb(min, max);
}

bool quat_rotate_x::hit(const ray& r, float t_min, float t_max, hit_record& rec) const {
	vec3 origin = r.ray_origin();
	vec3 direction = r.direction();
	origin[1] = cos_theta * r.ray_origin()[1] + sin_theta * r.ray_origin()[2];
	origin[2] = - sin_theta * r.ray_origin()[1] + cos_theta * r.ray_origin()[2];
